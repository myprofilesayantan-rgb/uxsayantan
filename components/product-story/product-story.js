...
window.ProductStoryModal = (function() {
  ...
  async function createMarkup() {
    ...
    try {
      const res = await fetch('components/product-story/product-story.html');
      if (res.ok) {
    ...
    for (let i = 1; i <= totalSlides; i++) {
      ...
      thumbnailsHTML += `
        <div class="thumbnail-card ${isLocked ? 'locked' : ''}" data-slide-index="${i - 1}">
          <img src="TRACTO WEB/${i}.png" alt="Thumb ${i}" />
    ...
    overlay.innerHTML = `
      ...
      <div class="story-theater-stage">
        ...
        <footer class="story-theater-footer">
          ...
          <span style="font-family: monospace; font-weight: 600;" id="story-counter-label">Slide 01 / 07</span>
        </footer>
      </div>
  ...
  function renderSlide(index, direction = 'next') {
    ...
    const imagePath = `TRACTO WEB/${index + 1}.png`;
  ...
  function bindEvents() {
    modalEl.addEventListener('click', (e) => {
      ...
      if (e.target.closest('#story-close-btn')) {
        const btn = e.target.closest('#story-close-btn');
        if (btn && btn.tagName === 'A') return; // Allow native link navigation on design-story.html
        closeModal();
        return;
      }
      ...
      if (e.target.closest('#story-dive-btn')) {
        const btn = e.target.closest('#story-dive-btn');
        if (btn && btn.tagName === 'A') return; // Allow native link navigation on design-story.html
        closeModal();
  ...
  return {
    init() {
      const existingModal = document.getElementById('product-story-modal');
      if (existingModal) {
        ...
        isAnimating = false;
        renderSlide(currentSlide);
        logClarityEvent('story_page_open');
      } else {
